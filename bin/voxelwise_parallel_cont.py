...
for key, inv_p_vals in results_dict.items():
    np.save(f"{output_dir}/group_{key}_chunk{chunk_id}_1minuspmap_cont.npy", np.array(inv_p_vals, dtype=np.float32))
...
for term in terms:
    ...
    for chunk_id in range(4):
        chunk = np.load(f"{output_dir}/group_{term}_chunk{chunk_id}_1minuspmap_cont.npy")
        chunks.append(chunk)
    full_data = np.concatenate(chunks)
    final_img = unmask(full_data, mask_img)
    final_img.to_filename(f"{output_dir}/group_{term}_1minuspmap_FULL_cont.nii.gz")
# for key, inv_p_vals in results_dict.items():
