...
cluster_1_path = f"{cluster_dir}/hip_cluster_no_acc_mask.nii.gz"
#cluster_1_path = f"{cluster_dir}/masked_hip_acc_clust.nii.gz"
# cluster_1 = nib.load(cluster_1_path).get_fdata().astype(bool)

cluster_2_path = f"{cluster_dir}/hip_cluster_no_acc_unmasked.nii.gz"
#cluster_2_path = f"{cluster_dir}/unmasked_hip_acc_clust.nii.gz"
...
cluster_index = 0
cluster_names = ['hip_cluster_no_acc_mask', 'hip_cluster_no_acc_unmasked']
for cluster_path in [cluster_1_path, cluster_2_path]:  #, cluster_3_path]: #, cluster_2, cluster_3, cluster_4, cluster_5]:
    ...
    for i, path in enumerate(subject_maps):
        ...
        out_path = f"{cluster_dir}/moshiGO_{sub}/moshiGO_{sub}_run-{run}_MASK_cluster-{cluster_names[cluster_index]}.nii.gz"
    ...
    df[f'cluster_{cluster_names[cluster_index]}_mean'] = means
    print("running model ...")
    model = smf.mixedlm(f"cluster_{cluster_names[cluster_index]}_mean ~ C(age_group) * run", df, groups=df["subject"])
    result = model.fit()
    print(f'RAN MODEL FOR VARIANCE EXPLAINED BY PC1 AND PC2 IN CLUSTER {cluster_names[cluster_index]}:')
    print()
