...
cluster_1 = nib.load(f"{cluster_dir}/cluster_001.nii.gz").get_fdata().astype(bool)
cluster_2 = nib.load(f"{cluster_dir}/cluster_002.nii.gz").get_fdata().astype(bool)
cluster_3 = nib.load(f"{cluster_dir}/cluster_003.nii.gz").get_fdata().astype(bool)

cluster_index = 1
for cluster_mask in [cluster_1, cluster_2, cluster_3]:
    means = []
