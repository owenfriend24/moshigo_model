...
# Load metadata
df = pd.read_csv("/home1/09123/ofriend/analysis/moshigo_model/pca_sl_meta_wb.csv")
...
# Load brain mask
mask_img = nib.load('/corral-repl/utexas/prestonlab/xmaze/batch/templates/graymatter.nii.gz')
...
for key, inv_p_vals in results_dict.items():
    np.save(f"{output_dir}/group_{key}_chunk{chunk_id}_1minuspmap_wb.npy", np.array(inv_p_vals, dtype=np.float32))
...
for term in terms:
    ...
    for chunk_id in range(4):
        chunk = np.load(f"{output_dir}/group_{term}_chunk{chunk_id}_1minuspmap_wb.npy")
        chunks.append(chunk)
    full_data = np.concatenate(chunks)
    final_img = unmask(full_data, mask_img)
    final_img.to_filename(f"{output_dir}/group_{term}_1minuspmap_FULL_wb.nii.gz")
# for key, inv_p_vals in results_dict.items():
